...
The existing Norwegian-South Saami lexical resources are poor, with little terminology from the modern society. South Saami has been used more for giving information to the Saami people especially the last two years, and there are some texts especially about school administration and curriculum on Internet. By comparing Norwegian texts with the translations to North Saami and South Saami, we added 873 special domain word pairs to the bilingual dictionary.

Large syntactic differences also have consequences for the bilingual dictionary, because many North Saami verbs have to be translated into South Saami object+verb og adverb+verb. Examples are Norwegian `å trekke lodd' is translated into North Saami \textit{vuorbádit} and North Saami \textit{vuerpiem giesedh}.
...
The pivot model causes some lexical problems. Even if the lexical inndeling often is similar in South Saami and North Saami, as in for the ‘to rain, to calcute’ pair, there are also counter examples, like the Norwegian verbs \textit{lese, telle, uttale} ‘read, count, utter’, which all three can be expressed with the North Saami verb \textit{lohkat}. But in South Saami is no common verb for these three, and the lexical xxx is like in Norwegian: \textit{lohkedh, ryöknehtidh, jiehtedh}. 


\subsubsection{Lexical selection}
Because we were tilpasse the MT system to a special domain, we have not made many rules for lexical selection. How many rules? But an MT-system for more domains, will certainly need more such rules. 
